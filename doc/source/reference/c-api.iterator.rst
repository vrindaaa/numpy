...
.. c:function:: NpyIter* NpyIter_MultiNew( \
    ...
    Flags that may be passed in ``op_flags[i]``, where ``0 <= i < nop``:
        ...
        .. c:var:: NPY_ITER_WRITEMASKED
           ...
           This array is the mask for all the `writemasked <numpy.nditer>`
           operands. Code uses the ``writemasked`` flag which indicates
           that only elements where the chosen ARRAYMASK operand is True will
           be written to. In general the iterator does not inforce this, it is
           up to the code doing the iteration to follow that promise.

            When ``writemasked`` is used, and this operand is buffered, this
